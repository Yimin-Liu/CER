...
from HCCL.utils.osutils import mkdir_if_missing
from HCCL.utils.serialization import write_json
...
def _pluck_msmt(list_file, subdir, pattern=re.compile(r'([-\d]+)_([-\d]+)_([-\d]+)')):
    ...
    for line in lines:
        ...
        pid, gid, cam = map(int, pattern.search(osp.basename(fname)).groups())
        if pid not in pids:
            pids.append(pid)
        # this_prefix = pid + gid + cam
        # if this_prefix not in all_img_prefix:
        #     all_img_prefix[this_prefix] = len(all_img_prefix)
        # img_idx = all_img_prefix[this_prefix]
