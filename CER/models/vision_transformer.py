...
class PatchMerging_column(nn.Module):
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.sie_embed = nn.Parameter(torch.zeros(6, 1, embed_dim))
    ...
    def forward_features(self, x, label=None, cam_label=None, view_label=None):
        B = x.shape[0]
        ...
        for blk in self.blocks:
            x = blk(x)
        # self.patch_partition_row(x)
        x = self.norm(x)

        return x[:, 0]

    def forward(self, x, label=None, cam_label=None, view_label=None):
        x = self.forward_features(x, label, cam_label, view_label)
